...
from utils.api_helpers import update_document_metadata as update_document
...
def analyze_document_with_openai(ocr_data, document_id):
    ...
    for attempt in range(max_retries):
        try:
            ...
            try:
                jsonschema.validate(
                    instance=json.loads(result), schema=tax_relief_schema
                )
                return result
            ...
            logging.warning(
                f"Rate limit exceeded (429 Too Many Requests). Retrying in {retry_after} seconds..."
            )
            time.sleep(retry_after)
        except openai.OpenAIError as e:
            if e.http_status == 428:
                logging.error(
                    "Precondition Required (428). Ensure the request meets the required conditions."
                )
                break
    ...
    for key, field in fields.items():
        try:
            if isinstance(field, str):
                ...
                ensure_custom_field_exists(
                    api_url, headers, field["name"], field["data_type"]
                )
            else:
                logging.warning(
                    f"Unexpected field mapping format for key '{key}': {field}"
                )
        except Exception as e:
            logging.error(
                f"Failed to ensure custom field for key '{key}' with field '{field}': {e}"
            )
...
def populate_field_mapping_with_ids(api_url, headers, field_mapping):
    ...
    try:
        ...
        normalized_custom_fields = {
            to_snake_case(name): data for name, data in custom_fields.items()
        }
        ...
        for key, field in field_mapping.items():
            if isinstance(field, dict):
                ...
                if field_name in normalized_custom_fields:
                    field_mapping[key]["id"] = normalized_custom_fields[field_name][
                        "id"
                    ]
                    field_mapping[key]["name"] = field_name  # Ensure name matches API
                    logging.debug(
                        f"Updated field mapping for key '{key}': {field_mapping[key]}"
                    )
                else:
                    logging.warning(
                        f"No matching custom field found for key '{key}' with name '{field_name}'"
                    )
            else:
                logging.warning(
                    f"Field mapping for key '{key}' is not a dictionary: {field}"
                )
        ...
        if missing_ids:
            logging.warning(
                f"The following keys are missing 'id' values: {missing_ids}"
            )
...
def update_document_with_json(api_url, headers, doc_id, json_data):
    ...
    processed_json_data = {
        key: (json.dumps(value, indent=4) if isinstance(value, (dict, list)) else value)
    ...
    combined_notes = (
        f"{analysis}\n\nDetected Services:\n{detected_services}"
        if analysis or detected_services
        else None
    )
