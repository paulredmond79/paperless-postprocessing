...
def create_correspondent(name):
    ...
    try:
        response = requests.post(f"{paperless_url}/api/correspondents/", headers=headers, json=payload)
        response.raise_for_status()
        correspondent = response.json()
        logging.debug(f"Response for created correspondent: {correspondent}")
        logging.info(f"Correspondent '{name}' created successfully with ID: {correspondent['id']}")
        return correspondent
    except requests.exceptions.HTTPError as e:
        if response.status_code == 400:
            logging.error(f"Failed to create correspondent '{name}'. Response: {response.text}")
            if "already exists" in response.text.lower():
                logging.warning(f"Correspondent '{name}' already exists. Fetching existing correspondent.")
                ...
                logging.warning(f"Correspondent '{name}' violates unique constraint. Searching for existing correspondent.")
                correspondents = fetch_correspondents()
                existing_correspondent = correspondents.get(name.lower())
                if existing_correspondent:
                    logging.info(f"Found existing correspondent '{name}' with ID: {existing_correspondent['id']}.")
                    return existing_correspondent
                else:
                    logging.error(f"Correspondent '{name}' not found despite unique constraint violation.")
                    raise
        raise
...
def create_tag(name):
    ...
    response = requests.post(f"{paperless_url}/api/tags/", headers=headers, json=payload)
    response.raise_for_status()
    tag = response.json()
...
def determine_correspondent_with_openai(ocr_data, correspondents):
    ...
    try:
        ...
        if not isinstance(ai_response_json, dict) or "status" not in ai_response_json:
            logging.error("Invalid AI response format. Expected a JSON object with a 'status' key.")
...
def update_document_correspondent(doc_id, correspondent_id):
    ...
    """
    logging.info(f"Updating document ID {doc_id} with correspondent ID {correspondent_id}.")
    doc_url = f"{paperless_url}/api/documents/{doc_id}/"
    ...
    response.raise_for_status()
    logging.info(f"Document ID {doc_id} updated with correspondent ID {correspondent_id} successfully.")
...
def main(doc_id):
    ...
    if ai_response["status"] == "match" or ai_response["status"] == "suggest_new":
        ...
        if not correspondent_name:
            logging.error("AI response is missing the 'correspondent' field for status 'match' or 'suggest_new'.")
            logging.debug(f"AI response content: {ai_response}")
            sys.exit(1)
    elif ai_response["status"] == "no_match":
        logging.warning("AI could not determine a correspondent. Adding 'gpt-correspondant-unable-to-determine' tag.")
        ...
        if not unable_to_determine_tag:
            logging.info(f"No matching tag found for '{unable_to_determine_tag_name}'. Creating a new one.")
        ...
        if unable_to_determine_tag["id"] not in current_tags:
            logging.info(f"Adding tag '{unable_to_determine_tag['name']}' (ID: {unable_to_determine_tag['id']}) to document ID {doc_id}.")
    ...
    if not correspondent:
        logging.info(f"No matching correspondent found for '{correspondent_name}'. Creating a new one.")
    ...
    if current_correspondent_id != correspondent["id"]:
        logging.info(f"Current correspondent ID ({current_correspondent_id}) differs from determined correspondent ID ({correspondent['id']}). Updating the document.")
        ...
        if tag["id"] not in current_tags:
            logging.info(f"Adding tag '{tag['name']}' (ID: {tag['id']}) to document ID {doc_id}.")
            add_tag_to_document(doc_id, tag["id"])
    else:
        logging.info(f"No update needed. Current correspondent ID ({current_correspondent_id}) matches the determined correspondent ID ({correspondent['id']}).")
        tag = None  # Ensure 'tag' is defined even if no updates are made

    # Check if OpenAI was unable to determine a correspondent
    if correspondent_name.lower() == "the ocr text does not provide a clear correspondent name.":
        logging.warning("OpenAI could not determine a correspondent. Adding 'gpt-correspondant-unable-to-determine' tag.")
    ...
    if tag:
        logging.info(f"Document {doc_id} processed successfully. Correspondent: {correspondent['name']}, Tag: {tag['name']}.")
    else:
        logging.info(f"Document {doc_id} processed successfully. Correspondent: {correspondent['name']}, No tag added.")
